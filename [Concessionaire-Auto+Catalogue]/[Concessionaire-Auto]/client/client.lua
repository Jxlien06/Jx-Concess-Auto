...
Citizen.CreateThread(function()
        ...
        SetBlipSprite(concessmap, 225)
        SetBlipColour(concessmap, 4)
        SetBlipScale(concessmap, 0.9)
...
local markerjob = {
        {x = -56.9, y = -1098.7, z = 26.39}, --point vente
}  
...
RMenu.Add('concessvente', 'main', RageUI.CreateMenu("Menu Concessionnaire", "Pour vendre des véhicules"))
    ...
    while concesspointvente do
           ...
           RageUI.ButtonWithStyle("Facture", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                if (Selected) then
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_concess', 'Facture Concessionnaire', amount)
                    end
                end
            end)
        ...
        RageUI.IsVisible(RMenu:Get('concessvente', 'categorievehicule'), true, true, true, function()
            ...
            if (Selected) then
					...
					ESX.Game.SpawnLocalVehicle(modelevoiture, {x = -49.1, y = -1094.7, z = 26.42}, 157.3, function (vehicle)
...
Citizen.CreateThread(function()
        while true do
            Citizen.Wait(0)
                ...
                local jobdist = Vdist(plycrdjob.x, plycrdjob.y, plycrdjob.z, -56.9, -1098.7, 26.39)
            if jobdist <= 1.0 then
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'concess' then  
                    ESX.ShowHelpNotification("Appuyez sur [~b~E~w~] pour accéder au menu Concessionnaire")
